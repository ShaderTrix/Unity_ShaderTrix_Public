...
public class UIOcclusionCullingManager : MonoBehaviour //Singleton<UIOcclusionCullingManager>
{
    [SerializeField] private Canvas _rootCanvasObject;
    ...
    void LateUpdate()
    {
        if (!_rootCanvasObject) return;
        if (_rootCanvasObject.transform.hierarchyCount != _lastHierarchyCount)
        {
            _lastHierarchyCount = _rootCanvasObject.transform.hierarchyCount;
            RefreshCanvases();
        }
    ...
    public void RequestOcclusion()
    {
        StartCoroutine(CalculateOcclusionCulling());
    }
    private IEnumerator CalculateOcclusionCulling()
    {
        yield return null;
        RebuildGraphicsAndBounds();
        RebuildQuadtree();
        CheckCollisionsViaQuadtree();
        Debug.Log("##OcclusionCulling Requested");
    }
    private void RefreshCanvases()
    {
        // if (_rootCanvasObject == null)
        // {
        //     _rootCanvasObject = FindObjectsByType<Canvas>(FindObjectsInactive.Exclude, FindObjectsSortMode.None)
    ...
    void RebuildGraphicsAndBounds()
    {
        ...
        foreach (var canvas in _canvases)
        {
            ...
            for (int i = 0; i < graphics.Count; i++)
            {
                ...
                if (!UIOcclusionRules.IsValidGraphic(g)) continue;
    ...
    void RebuildQuadtree()
    {
        ...
        const int objectsPerNode = 16;//each node can have 16 child nodes 
        const int maxDepth = 12; //can have uptil 12 layers of depth/subdivisions
        //after that it just bruteforce stores everything in the last node 
    ...
    void CheckCollisionsViaQuadtree()
    {
        ...
        var boundsA = new NativeArray<AABB2D>(count, Allocator.TempJob); //collects the bounding box of that object
        var keys = new NativeArray<long>(count, Allocator.TempJob); //collects depth of that object
        var results = new NativeArray<bool>(count, Allocator.TempJob); //result from the job

        for (int i = 0; i < count; i++) // get the data for each object 
        {
            boundsA[i] = ToAABB2D(_bounds[i]);
            keys[i] = GetRenderKey(_graphics[i]);
        }

        // MAIN THREAD: collect candidates from quadtree
        ...
        {
            var aAabb = boundsA[i];
            var list = new NativeList<int>(Allocator.Temp); //a lit for yet to be visited nodes 
            var dedupe = new NativeHashSet<int>(64, Allocator.Temp); // a list for visited nodes

            var collector = new CandidateCollector(ref list, ref dedupe);   //searches for worthy candidates for comparision
        ...
        //JOB: do containment + depth check in parallel
    ...
    //            a.max.y <= b.max.y + eps;
    // }
    static bool IsContained2D(AABB2D a, AABB2D b)
    {
        return b.Contains(a);
    }
