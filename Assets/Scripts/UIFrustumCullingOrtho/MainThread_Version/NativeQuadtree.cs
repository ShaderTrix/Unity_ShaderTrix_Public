...
namespace QuadTree_MainThreadVersion
{
    ...
    //<T> means it can be of any type (float,int,etc),unmanaged = something that is blittable type
    // (blittable means something that can be copied into memory without any handling)
    //structs are not handled by GC so we need to implement IDispossable manually..INativeDisposable is special to ECS/Jobs
    {
        ...
        //this is a linked function basically it means when we cll this it will just call the below fnction with the inserted default value
        //along with passing bounds,allocator forward it adds default value and calls the below fuction 
        {
        }
