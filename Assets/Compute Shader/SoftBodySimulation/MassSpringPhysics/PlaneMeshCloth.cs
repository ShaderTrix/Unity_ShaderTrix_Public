...
namespace SoftBodySimulation
{
    ...
    public sealed partial class PlaneMeshCloth : BaseClothSimulation
    {
        ...
        [SerializeField] private float _debugscale = 0.05f;
        private struct DebugSpring
        {
            public int a;
            public int b;
            public SpringDamperType type;
        }

        private readonly List<DebugSpring> _debugSprings = new();
        ...
        protected override void Start()
        {
            ...
            for (var z = 0; z <= amountOfDivisionsInMesh; ++z)
            {
                ...
                _springProcessor.AddSpringNode(boneObject.transform, radius);
                if(_debug)
                {
                    var debugSphere = GameObject.CreatePrimitive(PrimitiveType.Sphere);
                    debugSphere.transform.SetParent(boneObject.transform);
                    debugSphere.transform.localPosition = Vector3.zero;
                    debugSphere.transform.localScale = Vector3.one * _debugscale;
                    debugSphere.GetComponent<MeshRenderer>().material.SetColor("_BaseColor",Color.yellow);
            ...
            for (var z = 0; z <= amountOfDivisionsInMesh; ++z)
            {
                ...
                void TryAddSpringDamper(SpringDamperType type, int xx, int zz, float desiredDistance)
                {
                    ...
                    _springProcessor.AddSpringDamper(type, desiredDistance, myNodeIndex, otherNodeIndex);

                    if (_debug && (type == SpringDamperType.Elastic || type == SpringDamperType.Shear))
                    {
                        _debugSprings.Add(new DebugSpring
                        {
                            a = myNodeIndex,
                            b = otherNodeIndex,
                            type = type
        ...
        {
            if (!_debug || _springProcessor == null) return;

            foreach (var spring in _debugSprings)
            {
                var nodeA = _springProcessor.SpringNodeFor(spring.a);
                var nodeB = _springProcessor.SpringNodeFor(spring.b);

                if (nodeA == null || nodeB == null) continue;

                switch (spring.type)
                {
                    case SpringDamperType.Elastic:
                        Gizmos.color = Color.red;
                        break;

                    case SpringDamperType.Shear:
        ...
        public void Rotate()
        {
            ...
            for (var z = 0; z <= amountOfDivisionsInMesh; z++)
            {
                var springNode = _springProcessor.SpringNodeFor(CoordToIndex(x, z));
            ...
            _springProcessor.SyncPositionUpdatesImmediately();
        }
