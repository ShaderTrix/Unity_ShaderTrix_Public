...
namespace SoftBodySimulation
{
    ...
    /// </summary>
    public abstract class BaxterHandGrab : MonoBehaviour
    {                
        ...
        [SerializeField] private BaseClothSimulation cloth;

        private Vector3 RelativeClothPosition => cloth.transform.InverseTransformPoint(transform.position); //world - local position convertion
        ...
        //params means you can directly pass any ints it will automatically convert it into array
        //public bool DoesGrabberContainOneOfIndices(params int[] indices)
        //DoesGrabberContainOneOfIndices(1, 4, 9); //params must be the last argumnt thats the onlt rule
        {
            // No LINQ due to overhead constraints.
            //calling the delegte class "BaseCLothSimulation" which decies whose Enumerate to call GPUs or CPUs
        ...
        public void AttachToClosest()
        {
            ...
            var minSqrDistance = sqrCheckRadius;//length()
            ...
            foreach (var springNode in cloth.EnumerateNearbySphere(myPosition, checkRadius))
                ...
                {
                    //here we are doing springNodes(WS) - wholeCloth(WS) - hand(localSpace) ?? why this localSpace and not WS?
                    var difference = springNode.Position - offset - myPosition; //?? seems weird                     
                    var sqrDistance = difference.sqrMagnitude;

                    // Choose particle with smallest distance.
                    if (sqrDistance <= minSqrDistance)
                    {
                        if (ReferenceEquals(springNodesToAttachTo, null))//if (springNodesToAttachTo == null)
                            springNodesToAttachTo = new List<ISpringNode>();//actually create the list
                        else
            ...
            if (!ReferenceEquals(springNodesToAttachTo, null))//if (springNodesToAttachTo != null)
                AttachTo(springNodesToAttachTo.ToArray());//it adds a new component to the closed nodes 
        }
