...
namespace SoftBodySimulation
{
    ...
    public sealed class GpuClothSpringProcessor : IClothSpringProcessor
    {
        ...
        private ComputeBuffer _springNodeFrontBuffer;
        ...
        private readonly List<GPUSpringDamper> _springDampers;
        private GPUSpringNode[] _springNodes;
        private SpringNodeOutput[] _springNodeOutputs;
        private Vector3[] _initialPositions;
        private List<GPUSpringNode> _tempSpringNodeList;
        private readonly List<Transform> _springNodeBones;
        private ComputeBuffer _springDampersBuffer;
        private readonly MassSpringClothConfiguration _physicsWorldConfiguration;
        ...
        /// </summary>
        private Dictionary<int, List<GPUSpringDamper>> _temporarySpringDamperMapper;
        ...
        public GpuClothSpringProcessor(MassSpringClothConfiguration physicsWorldConfiguration, ComputeShader computeShader)
        {
            _physicsWorldConfiguration = physicsWorldConfiguration;
            _computeShader = computeShader;
            _springDampers = new List<GPUSpringDamper>();
            _tempSpringNodeList = new List<GPUSpringNode>();
            _springNodeBones = new List<Transform>();
            _temporarySpringDamperMapper = new Dictionary<int, List<GPUSpringDamper>>();
            ...
            _sentinelCuboidColliderComputeBuffer.SetData(new[] { new ImmovableCuboidCollisionAdapter(new Vector3(-99999f, -99999f, -99999f),Vector3.zero, Vector3.one) });
        }
        ...
        private void TransformDataIntoCorrectGpuFormat()
        {
            ...
            {
                //currentStart = 0
                // Node 0: slice = (0,2)   currentStart = 2
                // Node 1: slice = (2,6)   currentStart = 6
                // Node 2: slice = (6,9)   currentStart = 9
                //flattens the dictionaries key value pair into 1D array
        ...
        public void UpdateExternalForces(Transform windObj,float windStrength,float windNoiseScale)
        {
            _computeShader.SetFloat("_WindDir",windObj.transform.rotation.x);
            _computeShader.SetFloat("_WindStrength",windStrength);
            _computeShader.SetFloat("_WindSpatialScale",windNoiseScale);
            _computeShader.SetFloat("_Time",Time.time);
        }
        public void UpdateSpringDamperConstants()
        {
            _computeShader.SetFloat("_SpringDamping", _physicsWorldConfiguration.SpringDamping);
            var gravity = _physicsWorldConfiguration.Gravity;
            _computeShader.SetFloats("_Gravity", gravity.x, gravity.y, gravity.z);
            _computeShader.SetFloat("_InverseMass", _physicsWorldConfiguration.SpringInverseMass);
            _computeShader.SetFloat("_RestitutionConstant", _physicsWorldConfiguration.RestitutionConstant);
            _computeShader.SetFloat("_FrictionConstant", _physicsWorldConfiguration.FrictionConstant);

            _computeShader.SetBuffer(_forceKernel, "_SpringDamperBuffer", _springDampersBuffer);
            _computeShader.SetBuffer(_collisionKernel, "_SpringDamperBuffer", _springDampersBuffer);

            _computeShader.SetBuffer(_forceKernel, "_SpringNodesBuffer", _springNodeFrontBuffer);
            _computeShader.SetBuffer(_collisionKernel, "_SpringNodesBuffer", _springNodeFrontBuffer);

            _computeShader.SetBuffer(_forceKernel, "_SpringNodesOutputBuffer", _springNodeOutputComputeBuffer);
            _computeShader.SetBuffer(_collisionKernel, "_SpringNodesOutputBuffer", _springNodeOutputComputeBuffer);
        }
        /// <summary>
        ...
        private void SetupComputeShaderVariablesAndBuffers()
        {
            ...
            _computeShader.SetInt("_SpringNodeCount", _springNodes.Length);
            _computeShader.SetInt("_DeltaTimeDivisor", (int)_physicsWorldConfiguration.DeltaTimeDivisor);

            _springDampersBuffer = new ComputeBuffer(_springDampers.Count, Marshal.SizeOf<GPUSpringDamper>());
            _springDampersBuffer.SetData(_springDampers);
            // _computeShader.SetBuffer(_forceKernel, "_SpringDamperBuffer", _springDampersBuffer);
            // _computeShader.SetBuffer(_collisionKernel, "_SpringDamperBuffer", _springDampersBuffer);

            _springNodeFrontBuffer = new ComputeBuffer(_springNodes.Length, Marshal.SizeOf<GPUSpringNode>());
            // _computeShader.SetBuffer(_forceKernel, "_SpringNodesInputBuffer", _springNodeFrontBuffer);
            // _computeShader.SetBuffer(_collisionKernel, "_SpringNodesInputBuffer", _springNodeFrontBuffer);
        ...
        public void OnDestroy()
        {
            ...
            _springNodeFrontBuffer.Dispose();
        }
        ...
        {
            _tempSpringNodeList.Add(new GPUSpringNode(correspondingBoneTransform.transform.position, radius));
        ...
        public void AddSpringDamper(SpringDamperType type, float desiredDistance, int firstNodeIndex,
            int secondNodeIndex)
        {
            List<GPUSpringDamper> GetOrCreateList(int index)
            {
                if (!_temporarySpringDamperMapper.TryGetValue(index, out var list))
                    _temporarySpringDamperMapper.Add(index, list = new List<GPUSpringDamper>());
                return list;
            }
            ...
            var springConstant = _physicsWorldConfiguration.SpringConstantForTypes(type);

            firstNodeSpringDampers.Add(new GPUSpringDamper((uint)secondNodeIndex, desiredDistance,
                springConstant));
            secondNodeSpringDampers.Add(new GPUSpringDamper((uint)firstNodeIndex, desiredDistance,
                springConstant));
        }
        ...
        {            
            ...
            if (cuboidColliders.Length > 0)
            {
                ...
                _computeShader.SetBuffer(_collisionKernel, "_BoxCollidersBuffer", cuboidBuffer);
                _computeShader.SetInt("_BoxCollidersCount", cuboidColliders.Length);
            }
            else
            {
                _computeShader.SetBuffer(_collisionKernel, "_BoxCollidersBuffer", _sentinelCuboidColliderComputeBuffer);
                _computeShader.SetInt("_BoxCollidersCount", 0);
            }
            ...
            if (sphereColliders.Length > 0)
            {
                ...
                _computeShader.SetBuffer(_collisionKernel, "_SpheresColliderBuffer", sphereBuffer);
                _computeShader.SetInt("_SphereColliderCount", sphereColliders.Length);
            }
            else
            {
                _computeShader.SetBuffer(_collisionKernel, "_SpheresColliderBuffer", _sentinelSphereColliderComputeBuffer);
                _computeShader.SetInt("_SphereColliderCount", 0);
            }

            _computeShader.SetFloat("_DeltaTime", deltaTime / _physicsWorldConfiguration.DeltaTimeDivisor);
            
            void DispatchKernel(int kernel)
            {
                _springNodeFrontBuffer.SetData(_springNodes);
        ...
        public void OnDrawGizmos()
        {
            foreach (var springNode in _springNodes)
                for (var springDamperId = springNode.SpringDamper.Start;
                    springDamperId < springNode.SpringDamper.End;
                    ++springDamperId)
                {
